...
class PrescriptionManager:
    def __init__(self, global_vecs, global_lbls):
        self.g_vec = global_vecs
        self.g_lbl = global_lbls
...
def trinity_inference(img_crop, is_pill=True, custom_matrix=None, custom_labels=None):
    ...
    for name, vec_score in candidates:
        ...
        if total > best_score:
            best_score = total; final_name = clean_name
...
class AIProcessor:
    ...
    def loop(self):
        while not self.stopped:
            ...
            # 3. Group Box Calculation
            if pill_coords:
                counts = Counter(pill_names_batch)
                if counts:
                    majority_name, count = counts.most_common(1)[0]
                    all_x1 = [c[0] for c in pill_coords]
                    all_y1 = [c[1] for c in pill_coords]
                    all_x2 = [c[2] for c in pill_coords]
                    all_y2 = [c[3] for c in pill_coords]
...
def main():
    ...
    try:
        while True:
            ...
            cv2.putText(display, f"FPS: {fps:.1f} | Temp: {temp}", (10, 30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)

            frame_count += 1
            if time.time() - fps_time > 1.0: fps = frame_count; frame_count = 0; fps_time = time.time()
